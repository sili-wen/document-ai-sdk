...
import {
  ...
  DocumentContexts,
} from './resources/document-contexts';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['DOCUMENT_AI_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['DOCUMENT_AI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DOCUMENT_AI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Document AI API.
 */
export class DocumentAI {
   ...
   * API Client for interfacing with the Document AI API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['DOCUMENT_AI_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['DOCUMENT_AI_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DOCUMENT_AI_BASE_URL'),
    apiKey = readEnv('DOCUMENT_AI_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? DocumentAI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DOCUMENT_AI_LOG'), "process.env['DOCUMENT_AI_LOG']", this) ??
      defaultLogLevel;
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.DocumentAIError(
  ...
  static DocumentAI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DocumentAIError = Errors.DocumentAIError;
