...
```sh
npm install git+ssh://git@github.com:stainless-sdks/document-ai-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install document-ai`
...
1. Via the `DOCUMENT_AI_LOG` environment variable
...
const client = new DocumentAI({
  logger: logger.child({ name: 'DocumentAI' }),
...
const client = new DocumentAI({ fetch });
```
...
```ts
import DocumentAI from 'npm:document-ai';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/document-ai-typescript/issues) with questions, bugs, or suggestions.
